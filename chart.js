const loadData = d3.json('https://mbsoft.github.io/fleet_stats_updated.json').then(data => {
    const chartResultsData = data['chart']['result'][0];
    const quoteData = chartResultsData['indicators']['quote'][0];
  
    return chartResultsData['timestamp'].map((time, index) => ({
      date: new Date(time * 1000),
      data: quoteData['data'][index],
      sms: quoteData['sms'][index],
      rentals: quoteData['rentals'][index]
    }));
  });
  ...
  const initialiseChart = data => {
    data = data.filter(
      row => row['data'] && row['sms'] && row['rentals']
    );
  
    thisYearStartDate = new Date(2021, 0, 1);
    ...
    /* Volume series bars */
    const volData = data.filter(d => d['rentals'] !== null && d['rentals'] !== 0);
  
    const yMinVolume = d3.min(volData, d => {
      return Math.min(d['rentals']);
    });
  
    const yMaxVolume = d3.max(volData, d => {
      return Math.max(d['rentals']);
    });
